...
void CCensurDlg::ScanFilesAndReplace(const string& filePath, const string& censorDir, HWND hwnd)
{
	thread scanThread([this, filePath, censorDir, hwnd] {
		...
		if (!hProgressBar)
		{
			MessageBox(NULL, L"Прогресс-бар не найден", L"Ошибка", MB_OK);
			return;
		}
		...
		while (!stopScan && getline(file, line))
		{
			for (const auto& censorWord : censorWords)
			{
				...
				while (pos != string::npos)
				{
					...
					auto it = find_if(wordCount.begin(), wordCount.end(), [&](const auto& pair) {
						return pair.first == censorWord;
						});
					if (it != wordCount.end()) {
						it->second++;
					}
					else {
						wordCount.push_back({ censorWord, 1 });
					}
				}
			}
		...
		if (reportFile.is_open())
		{
			for (const auto& pair : wordCount)
			{
				reportFile << pair.first << " (" << pair.second << ")" << endl;
			}
			reportFile.close();
			MessageBox(NULL, L"Информация успешно записана в файл отчета", L"Успех", MB_OK);
		}
		...
		if (file.is_open())
		{
			...
			while (!stopScan && getline(file, line))
			{
				for (auto& pair : wordCount)
				{
					...
					{
						++pair.second;
						line.replace(pos, pair.first.length(), "***");
						pos = line.find(pair.first, pos + 3);
					}
				}
			...
			file.close();
			outFile.close();
			string reportFilePath = filePath + "_report.txt";
			ofstream reportFile(reportFilePath);
			if (reportFile.is_open())
			{
				for (const auto& pair : wordCount)
